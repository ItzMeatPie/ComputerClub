...
{
    public partial class Computers_Select : Form
    {

        private DataBase db = new DataBase();

        public Computers_Select()
        {
            InitializeComponent();
        }

        private void Computers_Select_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = "SELECT * FROM Компьютеры WHERE Номер_Компьютера = @Номер_Компьютера";
                ...
                using (SqlCommand command = new SqlCommand(sql, db.getConnection()))
                {
                    ...
                    else
                    {
                        ...
                        if (computersForm == null)
                        {
                            ...
                            computersForm = new ComputersForm();
                            computersForm.Show();
                        }

                        // Обновляем DataGridView на форме ComputersForm
                        computersForm.UpdateDataGridView(dataTable);
