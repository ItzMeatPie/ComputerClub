...
{
    public partial class CPUsForm : Form
    {
        ...
        private void CPUs_Load(object sender, EventArgs e)
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "_Орлов_Д_419_4_УПDataSet.Процессоры". При необходимости она может быть перемещена или удалена.
            this.процессорыTableAdapter.Fill(this._Орлов_Д_419_4_УПDataSet.Процессоры);
            // TODO: данная строка кода позволяет загрузить данные в таблицу "_Орлов_Д_419_4_УПDataSet.Процессоры". При необходимости она может быть перемещена или удалена.
            this.процессорыTableAdapter.Fill(this._Орлов_Д_419_4_УПDataSet.Процессоры);
            // TODO: данная строка кода позволяет загрузить данные в таблицу "_Орлов_Д_419_4_УПDataSet.Процессоры". При необходимости она может быть перемещена или удалена.
            this.процессорыTableAdapter.Fill(this._Орлов_Д_419_4_УПDataSet.Процессоры);
        ...
        {
            var cpu_add_form = new CPU_Add();
            cpu_add_form.Show();
            this.Hide();
        }

        private void CPUs_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void CPUs_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
        ...
        {
            var cpu_upd = new CPU_Update();
            cpu_upd.Show();
        }

        public void LoadData()
        {
            string sql = "SELECT * FROM Процессоры";
